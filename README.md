...
*NOTE*: This document has been tested and verified on Ubuntu. Contributions for other operating systems are welcome.
...
Then, remove the old generated files using following command:
```bash
make clean
```

After that, build the project and filesystem images using following command:
```bash
make && make fs.img
```
